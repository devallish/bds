...
insert into address_type (code, label) values
  ('home', 'Home'),
  ('postal', 'Postal'),
...
create table address (
  ...
  address_type_id text not null references address_type (code),
...
create policy "address_select_region_coordinator" on address
  for select
  using (get_my_role() = 'regional_coordinator' and region_of(person_id) = get_my_region());
